...
export async function onRequestPost(context) {
  ...
  for (const row of rows) {
    ...
    const qStatus = q.cleared ? 'CLEARED' : 'OPEN';
    ...
    if (!existing) {
      ...
      decision = {
        finalStatus: qStatus,
        finalDate: q.date || '',
        sourceFlag: 'NEW_FROM_EXCEL',
        syncNote: 'New comment received from latest Excel/query.'
      };
      ...
      await context.env.DB.prepare('INSERT INTO bitem_audit_log(action, bitem_id, fingerprint, username, display_name, role, details, created_at) VALUES(?,?,?,?,?,?,?,datetime(\'now\'))')
        .bind('NEW_FROM_EXCEL', displayId, fp, user.sub, user.name, user.role, JSON.stringify({ tp, stage: constructionStage(row), queryStatus: qStatus })).run();
    } else {
  ...
  if (isLast) {
    ...
    await context.env.DB.prepare(`
      UPDATE bitem_registry SET active=0, source_flag='REMOVED_FROM_EXCEL', sync_note='Deleted / not returned from latest Excel.', updated_at=datetime('now')
      WHERE active=1 AND last_sync_id<>?
    `).bind(syncId).run();
    for (const m of missingRows.slice(0, 500)) {
      await context.env.DB.prepare('INSERT INTO bitem_audit_log(action, bitem_id, fingerprint, username, display_name, role, details, created_at) VALUES(?,?,?,?,?,?,?,datetime(\'now\'))')
        .bind('REMOVED_FROM_EXCEL', m.bitem_id, m.fingerprint, user.sub, user.name, user.role, JSON.stringify({ note: 'Existing system comment did not return in latest Excel/query.', previousFinalStatus: m.final_status })).run();
    }
